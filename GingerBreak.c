...
static void find_vold()
{
	...
	//open execs on guest because this is not a /proc/self access
	...
	//vold pid in the guest is found
	vold.pid = found;
	vold.found = 1;
...
static int do_fault(uint32_t idx, int oneshot)
{
	...
	//comunicate with vold in the guest!
	n = sendmsg(sock, &msg, 0);
	if (n < 0 || oneshot) {
		close(sock);
		return n;
