 ...
 *     vulnerability was patched in the Android 2.3.6 OTA update.
 ...
 exploit succeeds on guest. host tries to spawn a shell
 but that shell has no extra privileges
...
get_symbol(char *name)
{
	...
	//returns an address from the guest kernel
	//since guest/host are identical, this addr
	//should be valid on the host as well
...
main(int argc, char **argv)
{
	...
	//opened on guest. fd is valid in guest
	...
	//ioctl exec'd on guest. guest kernel memory dumped
	...
	//will succeed in guest. getuid is transferred
	if (getuid() != 0) {
		printf("[-] privileges not escalated, exploit failed!\n");
		exit(1);
	}

	//nope, you didn't get anything bro.
	printf("[+] privileges escalated, enjoy your shell!\n");

	//shell exec;d on host, but its useless.
