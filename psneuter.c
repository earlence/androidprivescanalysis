...
int main(int argc, char **argv, char **envp)
{
    ...
    //will read /proc/self/environ so executes on host
	...
	*(strstr(workspace, ",")) = 0;
    ...
    //opens /proc on guest
    //we don't know if the app will access self later on and we could break a 
    //benign app this way
    dir = opendir("/proc");
    if(!dir)
    {
    ...
    //the adbdpid is not bound to this container, so the kill
    //is executed by the proxy on the guest killing adbd inside
    //the guest. since the ashmem pages are modified on the host
    //the exploit fails
